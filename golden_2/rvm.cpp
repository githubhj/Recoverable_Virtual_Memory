...
int get_file_size(string filepath){
	...
	//Read the file
	ifstream fp(filepath.c_str(), ios::binary | ios::ate);
...
void *rvm_map(rvm_t rvm, const char *segname, int size_to_create){
	...
	//If segment is present in the directory
		...
		if(segment_map.find(segname) == segment_map.end()){
			...
			char* data_buffer = new char[present_filesize + 1];

			file.read(data_buffer, present_filesize);
			cout<<"Data copied from Disk is "<< string(data_buffer)<<endl;
			memcpy(segment_map[segname]->base_addr, data_buffer, present_filesize);
...
void segment_truncate(rvm_t rvm_dir, const char* segment_name){
	...
	cout << segment_file << endl;

	//No log, just remove logfile even if it does not exist
	if(get_file_size(segment_logfile.c_str()) == -1){
		...
		while(getline(logfile_handler,line)){
			...
			if(pos != string::npos){
				...
				unsigned int len = strlen(segment_file.c_str());
				char* temp_segfile = (char*)malloc(len);
				memcpy(temp_segfile,segment_file.c_str(),len);

				ofstream segment_file_handler;
				segment_file_handler.open(temp_segfile, std::ios_base::in | std::ios_base::out| std::ios_base::binary);
